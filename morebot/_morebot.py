...
class Morebot:
    ...
    def get_inbounds(cls) -> Optional[Dict]:
        try:
            response = requests.get(url=f"{cls._base_url}/moreban/inbounds", timeout=cls._timeout)
    ...
    @classmethod
    def report_admin_usage(cls, db: Session, users_usage: List[Dict[str, Any]]) -> bool:
        if not users_usage:
            return True
        ...
        try:
            response = requests.post(
                f"{cls._base_url}/resellers/users/usages",
                json=merged_data,
